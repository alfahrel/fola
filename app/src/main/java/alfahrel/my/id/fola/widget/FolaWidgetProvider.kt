...
class FolaWidgetProvider : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ACTION_TOGGLE_TASK -> {
                ...
                if (taskId == -1L) return
                Thread {
                    FolaDatabase.getInstance(context).taskDao().setCompletedSync(taskId, !completed)
    ...
    companion object {
        ...
        fun updateWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
            val views      = RemoteViews(context.packageName, R.layout.widget_fola)
            ...
            // KEY FIX: add a unique data URI so Android recreates the factory
            // when the filter changes, instead of reusing the cached one
            val serviceIntent = Intent(context, FolaWidgetService::class.java).apply {
                putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId)
                data = android.net.Uri.parse(
                    "fola://widget/$appWidgetId?filter=${dateFilter}&done=${showDone}"
                )
            }
            views.setRemoteAdapter(R.id.widgetList, serviceIntent)

            val toggleTemplate = Intent(context, FolaWidgetProvider::class.java).apply {
        ...
        private fun loadTasks(context: Context, dateFilter: Int?, showCompleted: Boolean): List<Task> {
            val dao = FolaDatabase.getInstance(context).taskDao()
